    ...
    public partial class ConsultaMultasVehiculo : System.Web.UI.Page
    {
        ...
        private void CargarDatos()
        {
            try
            {
                ...
                var (pend, canc) =
                    objBL.ContarPorEstado(
                        txtCodigo.Text.Trim().ToUpper(),
                        ini,
                        fin);

                lblResumen.Text = $"Pendientes: {pend} | Canceladas: {canc}";
        ...
        protected void btnDescargar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DateTime ini = Convert.ToDateTime(txtFecIni.Text);
                DateTime fin = Convert.ToDateTime(txtFecFin.Text);
                var (pend, canc) = objBL.ContarPorEstado(txtCodigo.Text.Trim().ToUpper(), ini, fin);
                ...
                using (var pck = new ExcelPackage())
                {
                    ...
                    ws.Cells[4, 1].Value = $"Pendientes: {pend}  |  Canceladas: {canc}";
